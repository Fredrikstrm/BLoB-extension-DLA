...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        "--disable_blob_noise",
        action="store_true",
        help="Use pure LoRA (no BLoB noise, no KL).",
    )
...
# Change back to float64 when not running
...
def sample(self, status=True):
    if getattr(self, "use_frozen_blob_noise", False) or getattr(self, "disable_blob_noise", False):
        self.blobsample = False
        return
...
class BLoB(WrapperBase):
    ...
    def __init__(
        ...
        #---
        self.disable_blob_noise = getattr(args, "disable_blob_noise", False)

        if self.disable_blob_noise:
            # Pure LoRA: permanently disable blob noise
            for _, child in self.base_model.named_modules():
                if hasattr(child, "blobsample"):
                    child.blobsample = False
        #---
    ...
    def set_frozen_blob_noise(self, module, status: bool):
        """
        Enable/disable 'frozen BLoB noise' mode recursively.
        When enabled, each LoRA layer caches its first noise sample and reuses it.
        """
        for _, child in module.named_children():
            from peft.tuners.lora import LoraLayer  # or import at top

            if isinstance(child, LoraLayer):
                child.use_frozen_blob_noise = status
                if not status:
                    # clear any cached noise
                    for attr in ["frozen_r_A", "frozen_s_A", "frozen_lora_noise_a"]:
                        if hasattr(child, attr):
                            delattr(child, attr)
            else:
                self.set_frozen_blob_noise(child, status)
    ...
    # Adding lora freeze for mulitple forwards per generate.
    def _wrap_lora_layer(self, lora_layer):
        ...
        # NEW: frozen-noise flag
        lora_layer.use_frozen_blob_noise = False
