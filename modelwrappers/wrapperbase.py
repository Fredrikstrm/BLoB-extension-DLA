...
class WrapperBase(PeftModel):
    ...
    def fit_evaluate(self):
        ...
        if self.accelerator.is_local_main_process and self.wandb_logger is not None:
            ...
            save_folder = (
                f"checkpoints/{self.args.modelwrapper}/"
                f"{self.args.model}/{self.args.dataset}/{self.args.log_path}"
            )
            create_if_not_exists(save_folder)

            try:
                unwrapped = self.accelerator.unwrap_model(self.base_model)
            except Exception:
                unwrapped = self.base_model

            unwrapped.save_pretrained(save_folder)

            wandb.save(os.path.join(save_folder, "*"))
