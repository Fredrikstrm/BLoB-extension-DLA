...
def add_experiment_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of worker processes for DataLoader. Default is 0 (main process only).",
    )
    parser.add_argument(
        "--multi-answer",
        action="store_true",
        help="Whether examples can have multiple correct answers (for some QA datasets).",
    )
    parser.add_argument(
        "--subset-size",
        type=int,
        default=-1,
        help="Size of the subset of the dataset to use. Default is None (use all data).",
    )
    ...
    parser.add_argument(
        ...
        help="The model's backbone type. Options: 'seqcls' (e.g., RoBERTa for sequence classification), "
            ...
            "'seq2seq' (e.g., BART, T5 for sequence-to-sequence tasks).",
    )
    parser.add_argument(
        "--load-in-8bit",
        action="store_true",
        help="Whether to load the model weights in 8-bit precision for memory optimization.",
    )
...
def add_management_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--wandb-entity", 
        type=str, 
        default="fredrik-strom-technical-university-of-munich", 
