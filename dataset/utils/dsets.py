...
# Dialogue summarization (seq2seq) on knkarthick/dialogsum
class DialogueSumDataset(LMDataset):
    def __init__(
        self,
        tokenizer: AutoTokenizer,
        add_space: bool = False,
        max_seq_len: int = 512,
        multianswer: bool = False,  # kept for S2SDataset compatibility, unused
    ...
    def _format_prompts(self, batch):
        prompts = []
        for e in batch:
            prompts.append(f"Dialogue:\n{e['dialogue']}\n\nSummary:")
        return prompts

    def s2s_collate_fn(self, batch):
        # Evaluation loader: (inputs, targets, None)
        prompts = self._format_prompts(batch)
        model_inputs = self._tokenize_prompts_batch(prompts)
    ...
    def s2s_train_collate_fn_ori(self, batch):
        # Training loader: (inputs, targets)
        prompts = self._format_prompts(batch)
        model_inputs = self._tokenize_prompts_batch(prompts)
...
DialogueSum = DialogueSumDataset
dialoguesum = DialogueSumDataset